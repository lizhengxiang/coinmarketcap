...
func GetNumHoursMaxPrice(parameter *monitorType.GetNumHoursMaxPriceParameter) Coinmarketcap {
	...
	db.Where("created_at > ? AND created_at < ? AND cointype = ?", parameter.Past,parameter.Now,parameter.Cointype).
		Order("profit desc").First(&Coinmarketcap{}).Scan(&result)
	return result
}
...
func GetNumHoursMinPrice(parameter *monitorType.GetNumHoursMaxPriceParameter) Coinmarketcap {
	...
	db := databaseServer.GetDB()
	db.Where("created_at > ? AND created_at < ? AND cointype = ?", parameter.Past,parameter.Now,parameter.Cointype).
		Order("profit ASC").First(&Coinmarketcap{}).Scan(&result)
	return result
}
